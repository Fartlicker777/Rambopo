...
public class Rambopo : MonoBehaviour
{
    ...
    public GameObject NickLatkovichObject;
    ...
    public GameObject TwitchPlaysStuff;
    ...
    void Start()
    {
        GetComponent<KMBombModule>().OnActivate += delegate () { if (TwitchPlaysActive) TwitchPlaysStuff.SetActive(true); };
        var alphabet = "ABCDEFGHIJKLMNOPQRSTUVWXYZ";
        LeftFakePairs = Rnd.Range(3, 5);
        RightFakePairs = Rnd.Range(3, 5);
        _leftLetters = Enumerable.Range(0, 26).Select(i => alphabet[i].ToString()).ToArray().Shuffle().Take(LeftFakePairs + 1).ToArray();
        _rightLetters = Enumerable.Range(0, 26).Select(i => alphabet[i].ToString()).ToArray().Shuffle().Take(RightFakePairs + 1).ToArray();
        //LeftFakePairs = 3;
        //RightFakePairs = 3;
        Generate();
    }
    ...
    private IEnumerator NickLatkovich()
    {
        var duration = 30f;
        var elapsed = 0f;
        while (elapsed < duration)
        {
            NickLatkovichObject.transform.localPosition = new Vector3(0.075f, 0.00575f, Easing.InOutQuad(elapsed, 0.0475f, 0.0575f, duration));
            yield return null;
